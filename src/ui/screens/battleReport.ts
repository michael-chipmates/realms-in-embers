...
export function openBattleReport(screen: GameScreen, report: BattleReport): void {
  ...
  const content = h('div', { class: 'battle-body' },
    ...
    report.events.length > 0
      ? h('div', { class: 'battle-events' },
          ...report.events.map((e) => h('p', { class: `small battle-event battle-event-${e.kind}` }, e.text)))
      : null,
    report.captured
      ? h('p', { class: 'battle-captured small-caps' }, `${report.provinceName} changes hands.`)
      : null,
    h('div', { style: { display: 'flex', justifyContent: 'flex-end', marginTop: '0.6rem' } },
      h('button', { class: 'btn', onclick: () => modal.close() }, 'Close the account'),
    ),
  );
